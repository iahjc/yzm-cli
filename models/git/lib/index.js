...
const { readFile, writeFile, spinnerStart } = require('@yzm-cli2/utils');
...
const CloudBuild = require('@yzm-cli2/cloudbuild');

const DEFAULT_CLI__HOME = '.yzm-cli';
...
class Git {
    ...
    async prepare() {
        ...
        await this.checkRepo(); // 创建并检查远程仓库
        await this.checkGitIgnore(); // 检查并创建.gitignore文件
        await this.init(); // 完成本地仓库的初始化
    }
    ...
    async getCorrectVersion() {
        ...
        if (!releaseVersion) {
            ...
            const icnType = (await inquirer.prompt({
                type: 'list',
                name: 'icnType',
            ...
            const incVersion = semver.inc(releaseVersion, icnType);
            this.branch = `${VERSION_DEVELOP}/${incVersion}`;
            this.version = incVersion;
            console.log(incVersion);
        }
        log.verbose('本地开发分支', this.branch);
        // 3. 将version同步到package.json中
    ...
    async getRemoteBranchList(type) {
        ...
        if (type === VERSION_RELEASE) {
            ...
            reg = /.+?refs\/heads\/release\/(\d+\.\d+\.\d+)/g;
        }
    ...
    async checkRemoteMaster() {
        console.log(await this.git.listRemote(['--refs']));
...
*.sw?`);
    ...
    async checkGitToken() {
        ...
        if (!token || this.refreshToken) {
            log.warn(this.gitServer.type + ' token未生成', '请先生成' + this.gitServer.type + ' token ' + terminaLink('链接', this.gitServer.getTokenHelpUrl()));
    ...
    checkHomePath() {
        if (!this.homePath) {
            if (process.env.CLI_HOME_PATH) {
                ...
                this.homePath = path.resolve(userHome, DEFAULT_CLI__HOME);
            }
        }
